...
At the start of every session, fetch the latest version of this file from the repository before doing anything else. If the environment changes — new features are added, versions increment, TODOs are resolved, or the user explicitly asks — update this file on GitHub as part of that same session. The file is `SPACE_INSTRUCTIONS.md`, located at `ayoubharraby/falcon-tg/SPACE_INSTRUCTIONS.md`, and the direct link is: https://github.com/ayoubharraby/falcon-tg/blob/main/SPACE_INSTRUCTIONS.md
...
8. Update `SPACE_INSTRUCTIONS.md` to reflect the new version, any resolved TODOs, and any new architecture notes
